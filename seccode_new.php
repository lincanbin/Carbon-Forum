...
//$code_list = str_split("abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789");
$code_list = str_split("0123456789");

for ($i = 0; $i < $digit * 2; $i++) {
	$code_list[$i] = $code_list[mt_rand(0, count($code_list)-1)];
}
$verification_code =  substr(implode('', $code_list), 0, $digit);
...
$font_height = $height*mt_rand(7, 10)/10;
...
foreach ($code_list as $offset => $code) {
	...
	$background_color[$offset] = imagecolorallocate($background, 
		$material_design_color[$background_color_index][0],
		$material_design_color[$background_color_index][1],
		$material_design_color[$background_color_index][2]
	 );
	//画一个背景色块
	imagefilledrectangle($background, 
		$digit_width * $offset/2, 
		0, 
		$digit_width * ($offset/2+0.5), 
		$height, 
...
foreach ($code_list as $offset => $code) {
	...
	$font_color = imagecolorallocate($background, 
		$material_design_color[$font_color_index][0],
		$material_design_color[$font_color_index][1],
		$material_design_color[$font_color_index][2]
	 );
	imagettftext($background, 
		$font_height, 
		mt_rand(-25, 25), 
		$digit_width * $offset + mt_rand(0, $digit_width * 0.5), 
		mt_rand($font_height * 0.95, $font_height * 1.1), 
		$font_color, 
		dirname(__FILE__).'/includes/fonts/SourceCodePro-Black.ttf', 
		$code
	);
}
$noise_number = 0.1 * $digit_width * $height;
$line_number = min(0.1 * $height, 3);
$line_width = 0.07 * $height;
