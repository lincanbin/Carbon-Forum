...
$PostsArray = $DB->query("SELECT `ID`, `TopicID`,`UserID`, `UserName`, `Content`, `PostTime`, `IsDel` 
	...
	LIMIT " . ($Page - 1) * $Config['PostsPerPage'] . "," . $Config['PostsPerPage'], 
	array("id" => $ID)
);
if ($CurUserID) {
	$IsFavorite = $DB->single("SELECT ID 
		FROM " . $Prefix . "favorites 
		WHERE UserID=:UserID and Type=1 AND FavoriteID=:FavoriteID", 
		array(
			'UserID' => $CurUserID,
			'FavoriteID' => $ID
		)
...
if ($MCache) {
	...
	if($TopicViews && ($TopicViews-$Topic['Views']) >= 100){
		$DB->query("UPDATE " . $Prefix . "topics 
			FORCE INDEX(PRI) 
			SET Views = :Views,LastViewedTime = :LastViewedTime Where ID=:ID", 
			array(
				'Views' => $TopicViews+1,
	...
	$DB->query("UPDATE " . $Prefix . "topics 
		FORCE INDEX(PRI) 
		SET Views = Views+1,LastViewedTime = :LastViewedTime Where ID=:ID", 
		array(
			"LastViewedTime" => $TimeStamp,
			"ID" => $ID
		)
